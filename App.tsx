...
const App = () => {
    ...
    const renderSettingsMenu = () => (
        <>
            ...
            <button onClick={handleOpenPasswordReset} className="w-full text-left px-5 py-4 text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 border-b border-slate-200 dark:border-slate-800 flex items-center gap-3 transition-colors">
                <i className="fas fa-key text-slate-400 w-4 text-center"></i> Alterar Senha
            </button>

            <button onClick={handleLogout} className="w-full text-left px-5 py-4 text-xs font-bold text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 flex items-center gap-3 transition-colors">
                <i className="fas fa-sign-out-alt w-4 text-center"></i> Sair
            </button>
        </>
    );

    return (
        <div className="min-h-screen pb-20 relative bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
            {!isOnline && (
            ...
            {showPasswordResetModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in">
                    <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-2xl max-w-sm w-full border border-slate-200 dark:border-slate-800 text-center space-y-4">
                        <div className="w-16 h-16 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-500 rounded-full flex items-center justify-center mx-auto mb-2">
                        ...
                        <div className="flex gap-3 pt-4">
                            <button 
                                ...
                                className="flex-1 py-4 rounded-2xl font-bold text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors text-xs uppercase tracking-widest"
                            >
                                Cancelar
                            </button>
                            <button 
            ...
            <div className="max-w-4xl mx-auto p-4 md:p-8 animate-in">
                <header className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6 pt-4">
                    <div className="flex items-center justify-between w-full md:w-auto">
                        ...
                        <div className="md:hidden relative" ref={mobileSettingsRef}>
                            <button onClick={() => setIsSettingsOpen(!isSettingsOpen)} className="w-11 h-11 flex items-center justify-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-500 shadow-sm transition-colors">
                    ...
                    <div className="flex items-center gap-4">
                        <nav className="flex bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-1">
                            <button onClick={() => setView('routine')} className={`px-6 py-2.5 rounded-xl text-xs font-bold transition-all ${view === 'routine' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 dark:text-slate-500 hover:text-slate-900 dark:hover:text-slate-300'}`}>ROTINA</button>
                            <button onClick={() => setView('reviews')} className={`px-6 py-2.5 rounded-xl text-xs font-bold transition-all ${view === 'reviews' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 dark:text-slate-500 hover:text-slate-900 dark:hover:text-slate-300'}`}>REVISÕES</button>
                        </nav>
                ...
                <footer className="mt-16 text-center text-[10px] text-slate-400 dark:text-slate-600 font-black uppercase tracking-[0.4em] mb-12 italic">
